...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  loss = np.sum(- np.log(prob[np.arange(num_train), y])) # 0.006906s
  # loss = np.sum(- scores[np.arange(num_train), y] + np.log(np.sum(np.exp(scores), axis = 1))) # 0.006401s
